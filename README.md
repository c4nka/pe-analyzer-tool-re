<div align="center">
  <img src="https://images.seeklogo.com/logo-png/61/1/istinye-universitesi-logo-png_seeklogo-610039.png" width="300" alt="İstinye Üniversitesi Logosu">
</div>

# Gelişmiş PE Analiz ve Entropi Aracı (Advanced PE Analyzer)

![Python](https://img.shields.io/badge/Python-3.x-blue?style=flat-square&logo=python)
![Status](https://img.shields.io/badge/Status-In%20Development-orange?style=flat-square)
![License](https://img.shields.io/badge/License-MIT-green?style=flat-square)

## Proje Bilgileri
* **Kurum:** İstinye Üniversitesi
* **Ders:** Tersine Mühendislik (Reverse Engineering)
* **Danışman / Eğitmen:** Keyvan Arasteh Abbasabad
* **Geliştirici:** Raşit Çankaya

## İçindekiler
- [Proje Hakkında](#proje-hakkında)
- [Özellikler](#özellikler)
- [Mimari ve Dosya Yapısı](#mimari-ve-dosya-yapısı)
- [Kurulum ve Kullanım](#kurulum-ve-kullanım)

## Proje Hakkında
Bu araç, Windows PE (Portable Executable) dosyalarının statik analizini yapmak ve entropi (Shannon Entropy) algoritması kullanarak dosyaların paketlenmiş (packed) veya şifrelenmiş olma ihtimalini tespit etmek amacıyla geliştirilmektedir.

*(Not: Proje geliştirilme aşamasındadır. Mimari detaylar ve kullanım talimatları güncellenecektir.)*
