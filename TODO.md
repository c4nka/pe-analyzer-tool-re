# Geliştirme Bekleyen Görevler (TODO)

- [x] `analyzer/pe_parser.py`: İçe aktarılan fonksiyonların (Imports - DLL) ayrıştırılması için yeni bir metot yazılacak.
- [x] `analyzer/entropy.py`: Çok büyük dosyalar analiz edilirken entropi hesaplamasının performansı test edilecek.
- [x] `main.py`: Kullanıcı dostu hata mesajları geliştirilecek.
- [x] Rapor Dışa Aktarma (JSON/TXT Export) özelliği eklendi.
